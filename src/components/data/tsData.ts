...
export const tsB: TSItemList = [
  {
    ...
    problem: [
      ...
      "한쪽의 오디오가 켜지지 않는 현상이 발생하여 여러번 재시도 진행",
      "증상이 통화방에 나중에 들어온 사람에게서 높은 확률로 발생하여, 오디오의 publish 또는 입장 신호 처리 타이밍과 관련된 문제로 추정"
    ],
    solution: [
      "가설 A: 전화 화면에 마이크 버튼을 추가해 참여자가 임의로 오디오를 활성화 하도록 임시 대응했으나 해결이 되지 않아 폐기",
      "가설 B: iOS 제스처 제약을 의심해 Ready 버튼 클릭하여 사용자가 오디오 권한을 직접 허가하도록 하였으나 역시 해결이 되지 않고, 서비스 기능이 아닌 순수 Openvidu에서는 iOS에서도 정상작동 하였으므로 폐기",
      "가설 C: 인원 확인 시점을 connectionCreated에서 streamCreated로 옮겨봤으나 여전히 타이밍 불안정",
      "세 가설 모두 근본 원인을 확정하지 못해, 5초 후 오디오 활성화를 포기하고 입장 즉시 publishAudio: true로 오디오를 켜도록 수정"
    ],
    result: [
      "오디오 미출력 문제 해결: 타이머 기반 지연 발행 구조를 걷어내고 입장 즉시 publishAudio: true로 전환한 이후로는 참가자 입장 순서와 무관하게 오디오가 정상 발행, 다만 원래 의도했던 5초 대기 연출은 포기"
    ]
  }
];
...
export const tsC: TSItemList = [
  ...
  },
  {
    ...
    solution: [
      ...
      "스트리밍이 불가능한 axios 대신 websocket/sse/fetch 중에서 검토",
