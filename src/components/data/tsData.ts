...
export const tsA: TSItemList = [
  ...
  {
    title: "AI→BE→FE 전 레이어 추적으로 undefined.avgCount 크래시 원인 특정 및 해결",
    image: "",
    problem: [
      "AI 서버는 RTSP 스트림 다운, 서버 재시작, 프레임 캡처 간격 과대 설정 등으로 집계 데이터가 비면 해당 시간대는 BE로 아예 전송하지 않는 구조로 설계",
      "BE는 GROUP BY 쿼리로 시간대별 데이터를 집계하여 클라이언트로 전송하고, FE에서는 08시~07시 고정으로 차트에 표시하도록 설계",
      "위 세 구조가 겹쳐 없는 시간대 값에 접근할 때 undefined.avgCount TypeError 크래시 발생 — FE 코드만 봐서는 원인이 안 잡혀 AI/BE 코드까지 직접 확인해 역추적"
    ],
    solution: [
      "FE 차트 로직에서 없는 시간대 값을 nullish coalescing으로 기본값({ avgCount: 0, maxCount: 0 })으로 대체",
      "서비스 핵심 기능이 아니었고 첫 풀스택 프로젝트로 시간 배분이 중요했던 시점이라, FE 레이어에서 우선 대응"
    ],
    result: [
